...
import { combineLatest, map, filter, Observable, of, switchMap, withLatestFrom } from 'rxjs';
...
export class ViewerComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    const rotationSubscription = dragOnIncVol$.pipe(
      ...
      quat.mul(
        ...
        quat.setAxisAngle(
          quat.create(),
          axes0,
          -movementX * Math.PI / 180
        ),
        quat.setAxisAngle(
          quat.create(),
          axes1,
          movementY * Math.PI / 180
        )
      )
    ...
    const applyMatrixSub = this.store.pipe(
      select(outputs.selectors.incMatrixMat4)
    ).subscribe(v => {
