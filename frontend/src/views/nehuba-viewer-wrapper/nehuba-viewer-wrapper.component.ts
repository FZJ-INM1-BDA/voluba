...
export class NehubaViewerWrapperComponent implements OnDestroy, OnInit, AfterViewInit{
  @Input()
  layers: NehubaLayer[] = [{
    id: 'big brain grey value',
    url: 'precomputed://https://neuroglancer.humanbrainproject.org/precomputed/BigBrainRelease.2015/8bit',
    transform: [[1,0,0,-70677184],[0,1,0,-70010000],[0,0,1,-58788284],[0,0,0,1]]
  }, {
    id: 'bla',
    url: 'precomputed://https://neuroglancer.humanbrainproject.eu/precomputed/JuBrain/v2.2c/colin27_seg',
    transform: [[1.0, 0.0, 0.0, -75500000.0], [0.0, 1.0, 0.0, -111500000.0], [0.0, 0.0, 1.0, -67500000.0], [0.0, 0.0, 0.0, 1.0]]
  }]

  @Output()
  mousedownSliceView = new EventEmitter<{sliceView: export_nehuba.SliceView|null|undefined, event: MouseEvent}>()

  #onDestroyCb: (() => void)[] = []

  nehubaViewer: export_nehuba.NehubaViewer|null = null
  elementToSliceViewWeakMap = new WeakMap<HTMLElement, export_nehuba.SliceView>()
  ...
  ngAfterViewInit(): void {
    const subscription = merge(
      fromEvent<MouseEvent>(this.el.nativeElement, "mousedown", { capture: true }).pipe(
        map(ev => {
          return {
            sliceView: isHtmlElement(ev.target) ? this.elementToSliceViewWeakMap.get(ev.target) : null,
            event: ev
          }
        })
      ),
      fromEvent<MouseEvent>(this.el.nativeElement, "mouseup", { capture: true }).pipe(
        map(ev => ({ event: ev, sliceView: null }))
      )
    ).pipe(
  ...
  #patchNehuba() {
    ...
    const patchSliceViewPanel = (sliceViewPanel: export_nehuba.SliceViewPanel) => {  
