...
export class InputVolumesComponent implements OnDestroy{
  ...
  constructor(private store$: Store){
    this.#subscriptions.push(
      this.inputCtrl.valueChanges.subscribe(({ selectedTemplate, selectedIncoming }) => {
        if (selectedTemplate) this.store$.dispatch(inputs.actions.selectTemplate({ '@id': selectedTemplate}))
        if (selectedIncoming) this.store$.dispatch(inputs.actions.selecteIncoming({ '@id': selectedIncoming}))
      })
    )
  }
