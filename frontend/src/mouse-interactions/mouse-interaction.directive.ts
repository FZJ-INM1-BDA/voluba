...
type SupportedEvents =  "mousedown"  | "mouseup" | "click" | "mousemove" | "wheel"
type _Subject<T extends SupportedEvents> = Subject<{ eventname: T, event: GlobalEventHandlersEventMap[T] }>
...
export class MouseInteractionDirective implements OnDestroy, AfterViewInit{
  ...
  ngAfterViewInit(): void {
    ...
    const subscription = this.#getEventStream("mousedown").pipe(
      switchMap(() => this.#getEventStream("mousemove").pipe(
  ...
  #getEventStream<T extends SupportedEvents>(eventname: T) {
    if (!this.#eventsHandled.has(eventname)) {
      ...
      this.#htmlEl?.addEventListener(eventname, evh, { capture: true })
      this.#onDestroyCb.push(() => this.#htmlEl?.removeEventListener(eventname, evh, { capture: true }))
