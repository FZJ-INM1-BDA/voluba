...
export class Effects {
  ...
  rotationEffect = createEffect(() => this.actions$.pipe(
    ...
    map(([ input, incoming, flippedState, cXformMat ]) => {
      ...
      const { applyMirror, undoMirror } = getMirrorMat(flippedState, sizeVec3)
  ...
  setTranslateEffect = createEffect(() => this.actions$.pipe(
